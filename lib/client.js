window.__ModuleLoader__.load({
	id: "@dsh-external/dsh-bili-miniplay",
	factory: (require) => {
		var module = { exports: {} };
		...
		const LS_STATE = 'bili-miniplay-v2'
		const LS_HIST = 'bili-miniplay-hist-v1'
		const LS_PICS = 'bili-miniplay-pics-v1'
		const LS_KW = 'bili-miniplay-kw-v1'
		const LS_PROG = 'bili-miniplay-prog-v1'
		...
		function injectCss(css) {
			const tagId = '@dsh-external/dsh-bili-miniplay/bili.module.css'
			if (document.querySelector("style[data-plugin-css=\"" + tagId + "\"]") !== null) return
			const tag = document.createElement('style')
			tag.dataset.plugin = '@dsh-external/dsh-bili-miniplay'
...
.dy-login-body{padding:16px 16px 20px;display:flex;flex-direction:column;align-items:center;gap:12px}
.dy-login-qr-wrap{display:flex;flex-direction:column;align-items:center;gap:12px;width:100%}
.dy-qr{width:220px;height:220px;border-radius:10px;border:1px solid #eee;background:#fff;object-fit:contain}
.dy-qr-ph{width:220px;height:220px;border-radius:10px;background:#f7f7f7;display:flex;align-items:center;justify-content:center;color:#aaa;font-size:12px}
.dy-login-tip{font-size:12px;color:#666;text-align:center;line-height:1.5}
.dy-login-ok{display:flex;flex-direction:column;align-items:center;gap:14px;padding:24px 0}
.dy-hbtn.dy-login{color:#fb7299}
.dy-hbtn.dy-login.on{background:#fb7299;color:#fff}
...
`
		...
		function apply(ctx) {
			...
			function apiLoginStatus() {
				return fetch('/api/bili2/login/status').then(function (r) { if (!r.ok) throw new Error('http ' + r.status); return r.json() })
			}
			function apiLoginQr() {
				return fetch('/api/bili2/login/qr', { method: 'POST' }).then(function (r) { if (!r.ok) throw new Error('http ' + r.status); return r.json() })
			}
			function apiLoginPoll(key) {
				return fetch('/api/bili2/login/poll?key=' + encodeURIComponent(key)).then(function (r) { if (!r.ok) throw new Error('http ' + r.status); return r.json() })
			}
			function apiLoginLogout() {
				return fetch('/api/bili2/login/logout', { method: 'POST' }).then(function (r) { if (!r.ok) throw new Error('http ' + r.status); return r.json() })
			}
			...
			function Widget() {
				...
				const [loginImg, setLoginImg] = react.useState('')
				...
				// 播放进度（续播）相关：按视频 key 记录已观看秒数，持久化到 localStorage
				const progRef = react.useRef(prog0)
				const [progress, setProgress] = react.useState(progRef.current)
				const lastSaveRef = react.useRef(0)
				const startRef = react.useRef((saved0 && saved0.cur) ? (prog0[curKey(saved0.cur)] || 0) : 0)
				const progKeyRef = react.useRef((saved0 && saved0.cur) ? curKey(saved0.cur) : '')
				const playStartRef = react.useRef(Date.now())
				const playOffsetRef = react.useRef(startRef.current)
				const pausedRef = react.useRef(false)
				const endedFiredRef = react.useRef(false)
				const playerCurRef = react.useRef(0)
				const playerDurRef = react.useRef(0)
				const mutedRef = react.useRef(false)
				function setProg(key, sec) {
					if (!key) return
					const s = Math.max(0, Math.floor(sec || 0))
					progRef.current[key] = s
					setProgress(Object.assign({}, progRef.current))
				}
				...
				function doPlay(v, forceReload, resumeAt, muted) {
					if (!v) return
					...
					// resumeAt: 续播起始秒数（>0 时跳到该位置并从头重新挂载 iframe）
					const start = (resumeAt && resumeAt > 0) ? Math.floor(resumeAt) : 0
					if (forceReload || start > 0) seqRef.current++
					startRef.current = start
					progKeyRef.current = key
					playOffsetRef.current = start
					playStartRef.current = Date.now()
					pausedRef.current = false
					endedFiredRef.current = false
					setSt(function (s) { return Object.assign({}, s, { cur: v, kw: '' }) })
					// 直接基于时长设置结束计时，不再依赖 iframe 的 onLoad（onLoad 在缓存/复用等情况下可能不触发，会导致 deadline 永远为 0、永不重播/切下一个）
					const dur = parseDur(v.duration)
					armTimer(Math.max(0, dur - start), Date.now())
				}

				// 视频播放结束：优先由播放器 postMessage 的 ended 事件触发，定时器作为兜底
				function handleEnded() {
					if (endedFiredRef.current) return
					endedFiredRef.current = true
					endRef.current.deadline = 0
					const s = stRef.current
					const k = curKey(s.cur)
					// 播完后清除该视频的续播进度，下次从头开始
					if (k && progRef.current[k] != null) {
						delete progRef.current[k]
						setProgress(Object.assign({}, progRef.current))
						saveLS(LS_PROG, progRef.current)
					}
					progKeyRef.current = ''
					// 重播模式：即使未开启「自动播」也从 0 重新播放（“播完:重播”是独立开关）
					if (s.endMode === 'repeat' && s.cur) {
						doPlay(s.cur, true, 0, true)
						return
					}
					...
					if (next) doPlay(next, false, 0, true)
				}
				...
				function closeWin() { setHistView(false); setHelpView(false); setLoginView(false); stopPoll(); setOpen(false); setSt(function (x) { return Object.assign({}, x, { minimized: false }) }) }
				function minimizeWin() { setHistView(false); setHelpView(false); setLoginView(false); stopPoll(); setOpen(false); setSt(function (x) { return Object.assign({}, x, { minimized: true }) }) }
				// ===== 可选扫码登录 =====
				function stopPoll() { if (pollRef.current) { window.clearInterval(pollRef.current); pollRef.current = null } }
				function openLogin() {
					setHelpView(false); setHistView(false); setLoginView(true); setLoginMsg(''); setLoginImg('')
					apiLoginStatus().then(function (j) {
						if (j && j.loggedIn) { setLoggedIn(true); setLoginMsg('已登录 B 站') }
						else { setLoggedIn(false); genLoginQr() }
					}).catch(function () { setLoggedIn(false); genLoginQr() })
				}
				function genLoginQr() {
					setLoginBusy(true); setLoginMsg('正在生成二维码…'); setLoginImg('')
					apiLoginQr().then(function (j) {
						setLoginBusy(false)
						if (j && j.img) { setLoginImg(j.img); setLoginMsg('请用 B 站 App 扫码登录'); startPoll(j.qrcode_key) }
						else { setLoginMsg('二维码生成失败，请重试') }
					}).catch(function () { setLoginBusy(false); setLoginMsg('二维码生成失败') })
				}
				function startPoll(key) {
					stopPoll()
					if (!key) return
					pollRef.current = window.setInterval(function () {
						apiLoginPoll(key).then(function (j) {
							if (!j) return
							if (j.status === 'success') { setLoggedIn(true); setLoginMsg('登录成功'); stopPoll() }
							else if (j.status === 'scanned') { setLoginMsg('已扫码，请在手机上确认') }
							else if (j.status === 'expired') { setLoginMsg('二维码已过期，点击重新生成'); stopPoll() }
							else { setLoginMsg('请用 B 站 App 扫码登录') }
						}).catch(function () {})
					}, 2000)
				}
				function doLogout() {
					apiLoginLogout().then(function () { setLoggedIn(false); setLoginImg(''); setLoginMsg('已退出登录'); stopPoll() }).catch(function () { setLoginMsg('退出失败') })
				}
				function reopen() {
				...
				react.useEffect(function () {
					const t = window.setInterval(function () {
						...
						// 基于播放器真实 timeupdate 的结束检测：不依赖 API duration，也不依赖 ended 事件是否送达
						if (!endedFiredRef.current && playerDurRef.current > 0 && playerCurRef.current > 0 && playerCurRef.current >= playerDurRef.current - 2) {
							handleEnded()
						}
					}, 1000)
				...
				react.useEffect(function () {
					...
					function onMsg(e) {
						try {
							const d = e.data
							if (!d) return
							if (typeof d === 'string') { setLastMsg(d); return }
							if (typeof d === 'object') {
								const evt = d.event || d.cmd || d.command
								if (evt) {
									setLastMsg(String(evt))
									const pd = (d.data && typeof d.data === 'object') ? d.data : d
									if (evt === 'ended' || evt === 'complete' || evt === 'endedVideo') {
										handleEnded()
									} else if (evt === 'timeupdate' || evt === 'timeUpdate' || evt === 'progress' || evt === 'currentTime') {
										const cur = num(pd.currentTime != null ? pd.currentTime : (pd.current_time != null ? pd.current_time : (pd.time != null ? pd.time : pd.current)))
										const dur = num(pd.duration != null ? pd.duration : (pd.total != null ? pd.total : (pd.durationSec != null ? pd.durationSec : null)))
										if (dur != null && dur > 0) playerDurRef.current = dur
										if (!endedFiredRef.current && cur != null && cur >= 0) {
											playerCurRef.current = cur
											const k = progKeyRef.current || curKey(stRef.current.cur)
											if (k) {
												setProg(k, cur)
												playOffsetRef.current = cur
												playStartRef.current = Date.now()
											}
										}
										...
										const k = progKeyRef.current || curKey(stRef.current.cur)
										if (k) setProg(k, playOffsetRef.current + (Date.now() - playStartRef.current) / 1000)
									}
								} else if (d.data && typeof d.data === 'object' && d.data.event) {
									setLastMsg(String(d.data.event))
								}
							}
						} catch (err) {}
					}
				...
				// 全屏：CSS 层叠全屏（浏览器 Fullscreen API 对跨域 iframe 限制多，改用固定定位覆盖整个视口，100% 可用）
				function doFullscreen() {
					setFullscreen(function (x) { fullscreenRef.current = !x; return !x })
				}
				...
				react.useEffect(function () {
					function onKey(e) {
						...
						if (e.key === 'Escape') {
							// 全屏状态下 Esc 仅退出全屏，不触发插件关闭/返回
							if (fullscreenRef.current) { setFullscreen(false); fullscreenRef.current = false; return }
							if (document.fullscreenElement || document.webkitFullscreenElement) {
								const exit = document.exitFullscreen || document.webkitExitFullscreen
								if (exit) { try { exit.call(document) } catch (err) {} }
								return
							}
							if (loginView) { setLoginView(false); stopPoll(); return }
				...
				// 续播进度本地估算兜底：播放器未推送 timeupdate 时，按已播放时长累加
				react.useEffect(function () {
					const t = window.setInterval(function () {
						const k = progKeyRef.current
						if (!k || pausedRef.current || endedFiredRef.current) return
						const cur = playOffsetRef.current + (Date.now() - playStartRef.current) / 1000
						setProg(k, cur)
					}, 5000)
					return function () { window.clearInterval(t) }
				}, [])
				...
				react.useEffect(function () {
					return function () { saveLS(LS_PROG, progRef.current) }
				}, [])
				...
				const hideMain = (st.uiMode === 'mini' || st.uiMode === 'video' || histView || helpView || loginView)
				const hidePlayer = (histView || helpView || loginView)
				const winCls = 'dy-win' + (st.uiMode === 'mini' ? ' dy-win--mini' : '') + (st.uiMode === 'video' ? ' dy-win--video' : '') + (histView ? ' dy-win--hist' : '') + (loginView ? ' dy-win--hist' : '')
				const chips = st.tab === 'rec' ? REC_TOPICS : (st.tab === 'ai' ? AI_TOPICS : [])
				const isFollowTab = st.tab === 'follow'
				const curKeyStr = curKey(st.cur)
				const nxt = nextItem()
				const src = st.cur ? ('https://player.bilibili.com/player.html?' + (st.cur.bvid ? 'bvid=' + st.cur.bvid : 'aid=' + st.cur.aid) + (startRef.current > 0 ? '&t=' + startRef.current : '') + '&autoplay=' + (silentRef.current ? '0' : '1') + (mutedRef.current ? '&mute=1' : '') + '&high_quality=1&danmaku=1') : ''
				...
				function playerStyle() {
					if (fullscreen) return { position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', zIndex: 9999, background: '#000' }
				...
				const header = el('div', { className: 'dy-header' + (hideMain ? ' dy-hideable' : ''), onPointerDown: startDrag },
					...
					el('div', { className: 'dy-search' },
						...
						el('button', { className: 'dy-hbtn dy-login' + (loggedIn ? ' on' : ''), onClick: consume(openLogin) }, loggedIn ? '已登录' : '登录'),
				...
				const nextRow = el('div', { className: 'dy-nextrow' + (hideMain ? ' dy-hideable' : '') },
					...
					el('div', { className: 'dy-meta' },
						...
						el('button', { className: 'dy-mbtn', onClick: consume(function () { if (st.cur) doFullscreen() }) }, '全屏'),
						el('button', { className: 'dy-mbtn', onClick: consume(prevPlay) }, '上一个'),
						el('button', { className: 'dy-mbtn', onClick: consume(nextPlay) }, '下一个')
					)
				)

				const remain = (endRef.current && endRef.current.deadline > 0) ? Math.max(0, Math.ceil((endRef.current.deadline - Date.now()) / 1000)) : -1
				const playerStatus = el('div', { className: 'dy-nextrow' + (hideMain ? ' dy-hideable' : '') },
					el('span', { className: 'dy-nextinfo' },
						st.cur ? el('span', null, '播放器：' + lastMsg + ' · 结束剩' + (remain >= 0 ? remain + 's' : '—') + (playerDurRef.current ? ' · ' + Math.floor(playerCurRef.current) + '/' + Math.floor(playerDurRef.current) + 's' : '') + ' · 模式=' + st.endMode) : null
					)
				)
				...
				const miniCtrl = st.uiMode !== 'normal' ? el('div', { className: 'dy-mini-ctrl' },
					...
					el('button', { onClick: function (e) { e.stopPropagation(); doFullscreen() } }, '全屏'),
				...
				function onIframeLoad(e) {
					...
					const dur = parseDur(s.cur.duration)
					const remaining = Math.max(0, dur - (startRef.current || 0))
					armTimer(remaining, Date.now())
					// 重置本地播放进度基线（续播时从 startRef 处开始计秒）
					playStartRef.current = Date.now()
					playOffsetRef.current = startRef.current || 0
					pausedRef.current = false
				}
				...
				const histPanel = histView ? el('div', { className: 'dy-hist' },
					...
					el('div', { className: 'dy-hist-list' },
						(hist.length ? hist : persistHist).length ? (hist.length ? hist : persistHist).map(function (h, i) {
							const hk = curKey(h)
							const hpr = progRef.current[hk] || 0
							return el('div', { key: hk + '_h' + i, className: 'dy-item' + (hk === curKeyStr ? ' playing' : ''), onClick: function () { const pr = progRef.current[hk] || 0; doPlay(h, false, pr); setHistView(false) } },
								el('div', { style: { position: 'relative', flex: 'none' } },
									el(Pic, { url: h.pic }),
									hpr > 5 ? el('span', { style: { position: 'absolute', bottom: '3px', left: '3px', background: 'rgba(251,114,153,.92)', color: '#fff', fontSize: '9px', padding: '1px 5px', borderRadius: '4px', lineHeight: '1.3' } }, '续 ' + fmtDur(hpr)) : null
								),
								el('div', { className: 'dy-info' },
				...
				el('div', { className: 'dy-help-body' },
					el('div', { className: 'dy-help-sec' },
						...
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, 'F'), el('span', null, '播放器全屏')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-key' }, 'Esc'), el('span', null, '返回上一页 / 关闭 / 退出全屏'))
					),
					el('div', { className: 'dy-help-sec' },
						el('div', { className: 'dy-help-title' },
							el('svg', { viewBox: '0 0 24 24', width: '15', height: '15', fill: 'none', stroke: '#fb7299', strokeWidth: '2', strokeLinecap: 'round', strokeLinejoin: 'round', 'aria-hidden': 'true' },
					...
					el('div', { className: 'dy-help-sec' },
						el('div', { className: 'dy-help-title' },
							el('svg', { viewBox: '0 0 24 24', width: '15', height: '15', fill: 'none', stroke: '#fb7299', strokeWidth: '2', strokeLinecap: 'round', strokeLinejoin: 'round', 'aria-hidden': 'true' },
								...
								el('path', { d: 'M20.49 15a9 9 0 1 1-2.12-9.36L23 10' })
							),
							'重播 / 续播'
						),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-dot' }), el('span', null, '开启「自动播」后，单集放完按「播完」设置：下一个 / 重播')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-dot' }), el('span', null, '历史里的「续 mm:ss」：点一下从上次看到的位置接着放')),
						el('div', { className: 'dy-help-row' }, el('span', { className: 'dy-help-dot' }), el('span', null, '观看进度自动保存，重新打开插件也会续播'))
					),
					el('div', { className: 'dy-help-sec' },
						el('div', { className: 'dy-help-title' },
							el('svg', { viewBox: '0 0 24 24', width: '15', height: '15', fill: 'none', stroke: '#fb7299', strokeWidth: '2', strokeLinecap: 'round', strokeLinejoin: 'round', 'aria-hidden': 'true' },
						...
						el('div', { className: 'dy-help-link', onClick: function () { window.open('https://github.com/pyf2818/dsh-bili-miniplay', '_blank') } },
							...
							'github.com/pyf2818/dsh-bili-miniplay'
						),
				...
				const loginPanel = loginView ? el('div', { className: 'dy-hist' },
					el('div', { className: 'dy-hist-hd' },
						el('b', null, 'B 站登录'),
						el('button', { className: 'dy-hbtn', onClick: function () { setLoginView(false); stopPoll() } }, '返回')
					),
					el('div', { className: 'dy-login-body' },
						loggedIn
							? el('div', { className: 'dy-login-ok' },
								el('div', { className: 'dy-login-tip' }, loginMsg || '已登录'),
								el('button', { className: 'dy-retry-btn', onClick: consume(doLogout) }, '退出登录')
							)
							: el('div', { className: 'dy-login-qr-wrap' },
								loginImg ? el('img', { className: 'dy-qr', src: loginImg, alt: '登录二维码' }) : el('div', { className: 'dy-qr-ph' }, '二维码加载中'),
								el('div', { className: 'dy-login-tip' }, loginMsg || (loginBusy ? '等待扫码…' : '打开 B 站 App 扫码登录')),
								el('button', { className: 'dy-retry-btn', onClick: consume(genLoginQr) }, '重新生成')
							)
					)
				) : null
				...
				return el('div', { className: 'dy-root' },
					...
					open || (st.minimized && st.cur) ? el('div', { ref: winRef, className: winCls + (st.minimized ? ' dy-minimized-win' : ''), style: winStyle() },
						header, tabs, kwHist, chipsRow, opts, player, nextRow, playerStatus, list, histPanel, helpPanel, loginPanel,
			...
			ctx.inject(['slots'], function (scope) {
				return scope.slots.inject('shell.overlay', function () {
					return scope.slots.register(
						{ name: 'shell.overlay', id: 'bili-miniplay', order: 1000 },
